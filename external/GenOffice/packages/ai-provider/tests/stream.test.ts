...
describe('streamForProvider: a connection dropped mid tool arguments is not an empty stream', () => {
  ...
  it('gemini: preserves thought_signature on tool calls and echoes it in contents', async () => {
    const body = sseStream([
      'data: {"candidates":[{"content":{"parts":[{"functionCall":{"name":"insert_content","args":{"text":"hi"}},"thought_signature":"sig_123"}]},"finishReason":"STOP"}]}',
    ])
    let sentBody = ''
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation((_url: string, init?: RequestInit) => {
        if (init?.body) sentBody = String(init.body)
    ...
    expect(toolCalls).toHaveLength(1)
    expect(toolCalls[0].name).toBe('insert_content')
    expect(toolCalls[0].thoughtSignature).toBe('sig_123')
    expect(sentBody).toContain('"thought_signature":"sig_prev"')
  })
})
