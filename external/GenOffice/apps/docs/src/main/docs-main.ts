...
export function registerAiIpc(): void {
  ...
  ipcMain.handle('ai:stream', async (event, request: AiStreamRequest) => {
    ...
    try {
      ...
      const maxRetries = 5
      const delays = [2000, 4000, 7000, 11000, 15000]
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt < maxRetries && isAiOverloadedError(streamErr) && !controller.signal.aborted) {
            const delayMs = delays[attempt] || 15000
            console.warn(
              `[ai-stream] ${requestId} (${provider}/${config.model}) rate-limited/overloaded, retrying in ${delayMs}ms (attempt ${attempt + 1}/${maxRetries})...`,
            )
