...
interface AiPanelProps {
  ...
  /** D7.4: Save callback for local automation */
  onSaveDoc?: (targetName?: string) => Promise<{ ok: boolean; path?: string; error?: string }>
}
...
export function AiPanel({
  ...
  const initialDocContentSizeRef = useRef<number>(2)
  const initialDocTextLengthRef = useRef<number>(0)
  const automationTargetTitleRef = useRef<string | null>(null)
  const onSaveDocRef = useRef(onSaveDoc)
  onSaveDocRef.current = onSaveDoc
        ...
        onDone: ({ text, cancelled, turnLimit, truncated }) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendDone: (d: unknown) => void; sendCancelled: (c: unknown) => void; sendFileSaved: (s: unknown) => void; sendError: (e: unknown) => void } }).automationBridge) {
            ...
            if (cancelled) {
              ...
              const beforeLen = initialDocTextLengthRef.current
              const afterLen = editor.state.doc.textContent.trim().length
              const beforeSize = initialDocContentSizeRef.current
              const afterSize = editor.state.doc.content.size
              const childCount = editor.state.doc.childCount
              // D7.4 Strict Mutation Rule: textContent length >= 20 chars, content size > initial, childCount >= 1
              const hasContent = afterLen >= 20 && afterSize > beforeSize && childCount >= 1
              ...
              if (!hasContent) {
                ...
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: `${beforeLen} chars, size ${beforeSize}`,
                    after: `${afterLen} chars, size ${afterSize}`,
                    detail: `Docs content verification failed: text length (${afterLen}) < 20 or content size (${afterSize} <= ${beforeSize})`,
                  },
                })
              } else {
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: `empty (${beforeLen} chars, size ${beforeSize})`,
                    after: `populated (${afterLen} chars, size ${afterSize})`,
                    detail: `Document editor text length changed from ${beforeLen} to ${afterLen} chars (content size: ${beforeSize} -> ${afterSize})`,
                  },
                })

                if (onSaveDocRef.current) {
                  const rawTitle = automationTargetTitleRef.current
                  const targetName = rawTitle
                    ? (rawTitle.endsWith('.docx') ? rawTitle : `${rawTitle}.docx`)
                    : undefined
                  void onSaveDocRef.current(targetName).then((res) => {
                    activeAutomationJobIdRef.current = null
                    if (res.ok && res.path) {
                      ...
                      ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                        ...
                        message: res.error || 'Native saveDocxNew failed to save document to disk',
                      })
                    }
                  }).catch((err) => {
                    activeAutomationJobIdRef.current = null
  ...
  useEffect(() => {
    ...
    // Signal renderer ready once editor instance is mounted
    bridge.signalReady('document')
    ...
    const unsubStart = bridge.onStartAgent((payload) => {
      ...
      initialDocContentSizeRef.current = editor.state.doc.content.size
      initialDocTextLengthRef.current = editor.state.doc.textContent.trim().length
      ...
      runWith(instruction, payload.instruction, atts)
    })
