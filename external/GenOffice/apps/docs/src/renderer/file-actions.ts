...
/** D7.4: Direct native save for hidden background automation, returning the committed file path. */
export async function saveDocxForAutomation(
  ctx: FileActionContext,
  newDocName?: string,
): Promise<{ ok: boolean; path?: string; error?: string }> {
  try {
    const ok = await save(ctx, false, true, newDocName)
    const path = ctx.doc?.filePath ?? pathlessDocSavedPath
    if (ok && path) {
      return { ok: true, path }
    }
    return { ok: false, error: 'saveDocxNew did not return a valid file path' }
  } catch (err) {
