...
export function AiPanel({
  ...
  onSavePdf?: () => Promise<{ ok: boolean; path?: string; error?: string }>
}): ReactElement {
  ...
  const onSavePdfRef = useRef(onSavePdf)
  onSavePdfRef.current = onSavePdf
  const automationTargetTitleRef = useRef<string | null>(null)
  const createdDocumentPathRef = useRef<string | null>(null)
  /** Any tool in the current run reported mutated: true */
  const runMutatedRef = useRef(false)
  /** Active automation job id for D7.2 hidden background execution */
  ...
  if (!loopRef.current) {
    const deps: PdfAiDeps = {
      ...
      onDocumentCreated: (createdPath: string) => {
        createdDocumentPathRef.current = createdPath
      },
        ...
        onDone: ({ text, cancelled, turnLimit, truncated }) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendDone: (d: unknown) => void; sendCancelled: (c: unknown) => void; sendFileSaved: (s: unknown) => void; sendError: (e: unknown) => void } }).automationBridge) {
            ...
            if (cancelled) {
              ...
              // D7.4 PDF Dual-Pathway Content & Save Logic
              const createdPath = createdDocumentPathRef.current
              const hasInPlaceMutations =
                runMutatedRef.current &&
                ((apiRef.current.pendingSummary && apiRef.current.pendingSummary() !== '') ||
                  (apiRef.current.textInserts && apiRef.current.textInserts().length > 0) ||
                  (apiRef.current.formEdits && apiRef.current.formEdits().size > 0))

              if (createdPath) {
                // Branch 1: Standalone document created via create_document tool
                activeAutomationJobIdRef.current = null
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: 'initial blank PDF',
                    after: `standalone document created at ${createdPath}`,
                    detail: `Standalone document created via create_document tool at ${createdPath}`,
                  },
                })
                ;(window as unknown as { automationBridge: { sendFileSaved: (s: unknown) => void } }).automationBridge.sendFileSaved({
                  job_id: currentJobId,
                  file_path: createdPath,
                  file_format: 'pdf',
                  title: automationTargetTitleRef.current ?? undefined,
                })
              } else if (hasInPlaceMutations) {
                // Branch 2: In-place modifications to the active document
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  job_id: currentJobId,
                  ok: true,
                  turns: runToolsRef.current.length,
                  ...
                  content_detected: true,
                  content_signal: {
                    before: 'clean PDF buffer',
                    after: `mutations applied: ${apiRef.current.pendingSummary() || 'in-place edits'}`,
                    detail: `In-place PDF mutations verified (${runToolsRef.current.length} tools executed)`,
                  },
                })

                if (onSavePdfRef.current) {
                  void onSavePdfRef.current().then((res) => {
                    activeAutomationJobIdRef.current = null
                    if (res.ok && res.path) {
                      ;(window as unknown as { automationBridge: { sendFileSaved: (s: unknown) => void } }).automationBridge.sendFileSaved({
                        job_id: currentJobId,
                      ...
                      ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                        ...
                        message: res.error || 'Native pdfApi.save() failed',
                      })
                    }
                  }).catch((err) => {
                    activeAutomationJobIdRef.current = null
                ...
                // Rejection Invariant: Zero content generated (even if read/search tools were executed)
                activeAutomationJobIdRef.current = null
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  job_id: currentJobId,
                  ok: true,
                  ...
                  content_signal: {
                    before: 'clean PDF buffer',
                    after: `${runToolsRef.current.length} tools executed, 0 mutations`,
                    detail: 'PDF content verification failed: zero document mutations detected',
                  },
                })
              }
            }
  ...
  useEffect(() => {
    ...
    const unsubStart = bridge.onStartAgent((payload) => {
      ...
      if (payload.options) {
        ...
        if (opts.approx_pages) parts.push(`Approx Pages: ${opts.approx_pages}`)
        if (parts.length > 0) {
          instruction = `[PDF Directives]\n${parts.join('\n')}\n\n[User Instruction]\n${instruction}`
        }
      }
