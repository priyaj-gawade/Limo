...
import { AutomationViewManager } from './automation-view-manager'
import { AutomationExecutionRunner } from './automation-runner'
...
let automationRunner: AutomationExecutionRunner | null = null
let automationViewManager: AutomationViewManager | null = null
...
function createShellWindow(): void {
  ...
  // Wire D7.2 hidden background automation runner
  automationViewManager = new AutomationViewManager(win)
  automationRunner = new AutomationExecutionRunner(automationViewManager, automationJobManager, win)
  automationJobManager.setRunner(automationRunner.getRunner())
  ...
  win.on('closed', () => {
    ...
    if (automationViewManager) {
      automationViewManager.destroyActiveView()
      automationViewManager = null
      automationRunner = null
      automationJobManager.setRunner(null)
    }
  })
  ...
  } else if (existsSync(rendererHtml)) {
...
app.whenReady().then(async () => {
  ...
  startSheetsCaptureServer()
  automationServer.start().catch((err) => {
    console.error('[GenOffice Automation] Failed to start local server:', err)
  })
  automationServer.setDocumentOpener((filePath: string) => {
    revealShellWindow()
    return openGeneratedDocument(filePath)
  })
  createShellWindow()
...
app.on('before-quit', () => {
  ...
  markSheetsShuttingDown()
  stopSheetsSidecar()
  automationServer.stop().catch((err) => {
    console.error('[GenOffice Automation] Failed to stop local server:', err)
  })
})
