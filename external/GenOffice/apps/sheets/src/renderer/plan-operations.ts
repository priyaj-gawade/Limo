    ...
    try {
      const batch = workbookCommandBatchSchema.parse({
        ...
        operations: operations.map((op) => {
          if ('sheetId' in op && !(op as { sheetId?: string }).sheetId) {
            return { ...op, sheetId }
          }
          return op
        }),
      ...
      for (const operation of expandToPrimitiveOps(batch.operations, reader)) {
        ...
        if (
          ...
          const match = workbook
            .getSheets()
            .find((s) => s.getSheetName().toLowerCase() === (operation.sheetId as string).toLowerCase())
          if (match) {
            ;(operation as { sheetId: string }).sheetId = match.getSheetId()
          } else {
            return {
              ok: false,
