  ...
  return messages.map((m) => {
    ...
    if (m.role === 'assistant') {
      ...
      for (const call of m.toolCalls ?? []) {
        const sig = call.thoughtSignature || 'skip_thought_signature_validator'
        parts.push({
          functionCall: {
            name: call.name,
            args: call.input,
...
function extractThoughtSignature(part: unknown): string | undefined {
  if (!part || typeof part !== 'object') return undefined
  const p = part as Record<string, unknown>
  const fc = (p.functionCall && typeof p.functionCall === 'object' ? p.functionCall : {}) as Record<string, unknown>
  const sig =
    p.thought_signature ??
    p.thoughtSignature ??
    fc.thought_signature ??
    fc.thoughtSignature
  return typeof sig === 'string' && sig.trim().length > 0 ? sig.trim() : undefined
}
