...
export interface PdfAiDeps {
  ...
  /** D7.4: Callback when createDocument successfully creates a physical file */
  onDocumentCreated?(filePath: string): void
...
export async function executePdfTool(
  ...
  switch (call.name) {
    ...
    case 'create_document': {
      ...
      if (r.path && deps.onDocumentCreated) {
        deps.onDocumentCreated(r.path)
      }
