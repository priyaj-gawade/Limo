...
import {
  handleExportCsv as handleExportCsvImpl,
  activeCsvSheet,
...
export function App(): React.JSX.Element {
  ...
  /** Active automation job id for D7.2 hidden view execution */
  const activeAutomationJobIdRef = useRef<string | null>(null)
  const initialKnownCellsRef = useRef<number>(0)
  const initialJournalEntriesRef = useRef<number>(0)
  const automationTargetTitleRef = useRef<string | null>(null)
  ...
  if (!agentLoopRef.current) {
    agentLoopRef.current = new AgentLoop({
      ...
      events: {
        ...
        onToolStart: (call) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendProgress: (p: unknown) => void } }).automationBridge) {
            (window as unknown as { automationBridge: { sendProgress: (p: unknown) => void } }).automationBridge.sendProgress({
              job_id: activeAutomationJobIdRef.current,
              substate: 'RUNNING_AGENT',
              current_turn: runToolsRef.current.length + 1,
              max_turns: 100,
              last_tool: call.name,
        ...
        onToolExecuted: ({ call, execution }) => {
          ...
          console.log(`[Sheets onToolExecuted] tool: ${call.name}, mutated: ${execution.mutated}, isError: ${execution.isError}`)
          ...
          if (execution.isError) {
            console.error(`[Sheets Tool Error] ${call.name}: ${execution.summary} | Output: ${execution.output}`)
          }
          patchLastAssistant((entry) => {
        ...
        onDone: async ({ text, cancelled, turnLimit, truncated }) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendDone: (d: unknown) => void; sendCancelled: (c: unknown) => void; sendFileSaved: (s: unknown) => void; sendError: (e: unknown) => void } }).automationBridge) {
            const currentJobId = activeAutomationJobIdRef.current
            if (cancelled) {
              activeAutomationJobIdRef.current = null
              ;(window as unknown as { automationBridge: { sendCancelled: (c: unknown) => void } }).automationBridge.sendCancelled({
                job_id: currentJobId,
              })
            } else {
              // Await any pending plan applies before verifying content
              if (aiApplyPromisesRef.current.length > 0) {
                await Promise.all(aiApplyPromisesRef.current)
              }
              ...
              try {
                ...
                finalCells = info.knownAddresses?.length ?? 0
              } catch {
                finalCells = 0
              }
              const finalJournal = lazyWorkbookRef.current
                ? journalSize(lazyWorkbookRef.current.editJournal)
                : 0
              const beforeCells = initialKnownCellsRef.current
              const beforeJournal = initialJournalEntriesRef.current

              // D7.4 Strict Mutation Rule: cells > 0 or journal > 0, and runMutatedRef === true
              const hasContent = (finalCells > 0 || finalJournal > 0) && runMutatedRef.current
              console.log(`[Sheets onDone] hasContent=${hasContent}, runMutated=${runMutatedRef.current}, finalCells=${finalCells}, finalJournal=${finalJournal}`)

              if (!hasContent) {
                activeAutomationJobIdRef.current = null
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  job_id: currentJobId,
                  ok: true,
                  turns: runToolsRef.current.length,
                  tools_executed: runToolsRef.current.map((t) => t.name),
                  content_detected: false,
                  content_signal: {
                    before: `cells: ${beforeCells}, journal: ${beforeJournal}`,
                    after: `cells: ${finalCells}, journal: ${finalJournal}`,
                    detail: `Sheets content verification failed: runMutatedRef=${runMutatedRef.current}, finalCells=${finalCells}, finalJournal=${finalJournal}`,
                  },
                })
              } else {
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: `cells: ${beforeCells}, journal: ${beforeJournal}`,
                    after: `cells: ${finalCells}, journal: ${finalJournal}`,
                    detail: `Workbook cells: ${beforeCells} -> ${finalCells}, edit journal entries: ${beforeJournal} -> ${finalJournal}`,
                  },
                })

                // D7.4: Trigger native save
                if (lazyWorkbookRef.current) {
                  void handleSaveRef.current('save', true).then(() => {
                    activeAutomationJobIdRef.current = null
                    const savedPath = lazyWorkbookRef.current?.file.path
                    if (savedPath) {
                      ;(window as unknown as { automationBridge: { sendFileSaved: (s: unknown) => void } }).automationBridge.sendFileSaved({
                        job_id: currentJobId,
                        file_path: savedPath,
                        file_format: 'spreadsheet',
                        title: automationTargetTitleRef.current ?? undefined,
                      })
                    } else {
                      ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                        job_id: currentJobId,
                        code: 'SAVE_EXECUTION_FAILED',
                        message: 'Sheets handleSave succeeded but lazyWorkbook file path is missing',
                      })
                    }
                  }).catch((err) => {
                    activeAutomationJobIdRef.current = null
                  ...
                  // In-memory Univer mode: export active sheet to native XLSX via createDocument
                  const active = activeCsvSheet(univerRef.current)
                  const csvContent = active ? serializeActiveSheetCsv(active.sheet, null) : null
                  if (csvContent && csvContent !== 'too-large' && window.desktopApi?.createDocument) {
                    void window.desktopApi.createDocument({
                      type: 'xlsx',
                      title: automationTargetTitleRef.current || 'Spreadsheet',
                      content: csvContent,
                    }).then((res) => {
                      activeAutomationJobIdRef.current = null
                      if (res.ok && res.path) {
                        ...
                        ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                          job_id: currentJobId,
                          code: 'SAVE_EXECUTION_FAILED',
                          message: res.error || 'Failed to export in-memory spreadsheet',
                        })
                      }
                    }).catch((err) => {
                      activeAutomationJobIdRef.current = null
                    ...
                    ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                      job_id: currentJobId,
                      code: 'SAVE_EXECUTION_FAILED',
                      message: 'Could not serialize active sheet in demo mode',
                    })
                  }
                }
              }
        ...
        onError: (error) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendError: (e: unknown) => void } }).automationBridge) {
            const currentJobId = activeAutomationJobIdRef.current
            activeAutomationJobIdRef.current = null
            ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
              job_id: currentJobId,
  ...
  // ── D7.2 Hidden Background Automation Bridge ─────────────────────────────
  useEffect(() => {
    const bridge = (window as unknown as { automationBridge?: import('@genoffice/electron-utils').AutomationBridge })
      .automationBridge
    if (!bridge) return

    bridge.signalReady('spreadsheet')

    const unsubStart = bridge.onStartAgent((payload) => {
      activeAutomationJobIdRef.current = payload.job_id
      automationTargetTitleRef.current = typeof payload.options?.title === 'string' ? payload.options.title : null
      try {
        const info = getActiveSheetInfo()
        initialKnownCellsRef.current = info.knownAddresses?.length ?? 0
      } catch {
        initialKnownCellsRef.current = 0
      }
      initialJournalEntriesRef.current = lazyWorkbookRef.current
        ? journalSize(lazyWorkbookRef.current.editJournal)
        : 0

      const atts: AttachmentMeta[] = (payload.attachments || []).map((a) => ({
        path: a.storage_ref,
        name: a.filename,
      ...
      if (payload.options) {
        ...
        if (opts.sheets_structure) parts.push(`Sheets Structure: ${JSON.stringify(opts.sheets_structure)}`)
        if (opts.tone) parts.push(`Tone: ${opts.tone}`)
        if (parts.length > 0) {
          instruction = `[Spreadsheet Directives]\n${parts.join('\n')}\n\n[User Instruction]\n${instruction}`
        }
      }

      runAgent(instruction, atts)
    })
