...
function parseReadSheetId(
  ...
  const matchedById = info.sheets.find((sheet) => sheet.id === sheetId)
  if (matchedById) return { sheetId: matchedById.id }
  const matchedByName = info.sheets.find((sheet) => sheet.name.toLowerCase() === sheetId.toLowerCase())
  if (matchedByName) return { sheetId: matchedByName.id }
  return {
...
export function executeWorkbookTool(
  ...
  switch (call.name) {
    ...
    case 'propose_operations': {
      ...
      const activeSheetId = deps.getActiveSheetInfo().sheetId
      const fallbackSheetId =
        (rawOps.find(
          (op) =>
            op &&
            typeof op === 'object' &&
            typeof (op as Record<string, unknown>).sheetId === 'string' &&
            (op as Record<string, unknown>).sheetId,
        ) as Record<string, unknown> | undefined)?.sheetId as string | undefined || activeSheetId

      const preparedOps = rawOps.map((op: unknown) => {
        if (op && typeof op === 'object') {
          const record = op as Record<string, unknown>
          if ((!record.sheetId || typeof record.sheetId !== 'string') && fallbackSheetId) {
            return { ...record, sheetId: fallbackSheetId }
          }
        }
        return op
      })
      ...
      try {
        operations = z.array(workbookOperationSchema).parse(preparedOps)
      } catch (e) {
