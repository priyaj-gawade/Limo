...
export function AiPanel({
  ...
  const initialSlideCountRef = useRef<number>(0)
  const initialElementCountRef = useRef<number>(0)
  ...
  if (!loopRef.current) {
    // The three slides generation steps (style/planning/per-page HTML) force the cost-efficient Gemini Flash Lite model;
    ...
    const SLIDES_GEN_MODEL = 'gemini-3.5-flash-lite'
    ...
    const settingsForGen = (): AiSettings => {
      ...
      if (cur.provider !== 'gemini') return cur
      const gp = cur.providers.gemini
      return {
        ...cur,
        providers: { ...cur.providers, gemini: { ...gp, model: SLIDES_GEN_MODEL } },
      }
    }
    ...
    const runLlmOnce = async (
      ...
      const cur = settingsRef.current
      if (cur.provider !== 'gemini') return first // The gen-model override only applies with gemini
      if (cur.providers.gemini?.model === SLIDES_GEN_MODEL) return first
        ...
        onDone: ({ text, cancelled, turnLimit, truncated }) => {
          if (activeAutomationJobIdRef.current && (window as unknown as { automationBridge?: { sendDone: (d: unknown) => void; sendCancelled: (c: unknown) => void; sendFileSaved: (s: unknown) => void; sendError: (e: unknown) => void } }).automationBridge) {
            ...
            if (cancelled) {
              ...
              const beforeSlides = initialSlideCountRef.current
              const beforeElements = initialElementCountRef.current
              const afterSlides = slidesRef.current.length
              const afterElements = slidesRef.current.reduce(
                (acc, s) => acc + (s.nodes?.length || 0),
                0,
              )
              // D7.4 Strict Mutation: slides >= 1, total elements >= 1, and non-empty element content
              const hasContent = afterSlides >= 1 && afterElements >= 1 && slidesRef.current.some((s) => (s.nodes || []).length > 0)
              ...
              if (!hasContent) {
                ...
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: `slides: ${beforeSlides}, elements: ${beforeElements}`,
                    after: `slides: ${afterSlides}, elements: ${afterElements}`,
                    detail: `Slides content verification failed: slides (${afterSlides}) < 1 or elements (${afterElements}) < 1`,
                  },
                })
              } else {
                ;(window as unknown as { automationBridge: { sendDone: (d: unknown) => void } }).automationBridge.sendDone({
                  ...
                  content_signal: {
                    before: `slides: ${beforeSlides}, elements: ${beforeElements}`,
                    after: `slides: ${afterSlides}, elements: ${afterElements}`,
                    detail: `Slide deck elements/nodes: ${beforeElements} -> ${afterElements} (slides: ${beforeSlides} -> ${afterSlides})`,
                  },
                })

                // D7.4: Trigger native save
                void (window as unknown as { slidesApi: { save: () => Promise<{ ok: boolean; path?: string; error?: string }> } }).slidesApi.save().then((res) => {
                  activeAutomationJobIdRef.current = null
                  if (res.ok && res.path) {
                    ...
                    ;(window as unknown as { automationBridge: { sendError: (e: unknown) => void } }).automationBridge.sendError({
                      ...
                      message: res.error || 'Native slidesApi.save() failed',
                    })
                  }
                }).catch((err) => {
                  activeAutomationJobIdRef.current = null
  ...
  useEffect(() => {
    ...
    bridge.signalReady('presentation')
    ...
    const unsubStart = bridge.onStartAgent((payload) => {
      ...
      initialSlideCountRef.current = slidesRef.current.length
      initialElementCountRef.current = slidesRef.current.reduce(
        (acc, s) => acc + (s.nodes?.length || 0),
        0,
      )
      ...
      if (payload.options) {
        ...
        if (opts.approx_pages) parts.push(`Target Units/Pages: ${opts.approx_pages}`)
        if (opts.tone) parts.push(`Tone: ${opts.tone}`)
        if (Array.isArray(opts.sections_outline) && opts.sections_outline.length > 0) {
          parts.push('Outline:\n' + opts.sections_outline.map((s: string, i: number) => `  ${i + 1}. ${s}`).join('\n'))
        }
        if (parts.length > 0) {
          instruction = `[Document Directives]\n${parts.join('\n')}\n\n[User Instruction]\n${instruction}`
        }
      }

      runWith(instruction, payload.instruction, { attachments: atts })
    })
